...
## 14c. Source expansion (operator decision 2026-07-07 — design locked, build queued)

Three new observation sourceTypes beyond documents/CH: **`note`** (operator notes — tier ~3, deliberate human record; hook note.create/update; build FIRST — smallest, highest signal), **`meeting_transcript`** (Fireflies — tier 1–2, hook post-sync per transcript; conversation-tuned gate: commitments/stated appetite/decisions/corrections only; speaker-anchored statements; externalRef = meeting id), **`email`** (inbound replyEvents hook — tier 0–1; knowledge-enabled clients only; tightest gate: commitments/figures/stated facts, never sentiment/logistics; most emails yield zero atoms by design; documentary tiers supersede as deals progress). Schema: additive literals on atomObservations.sourceType. UI: drawer gains a "colour by: family | source" toggle + source filter chips (trust gradient visible: document/CH/transcript/email/note distinct hues). Scale posture: the three gates carry the load; 2c IDF/salience downweights conversational chatter in ranking. Emails ship LAST with a gate-behavior review on a sample corpus before going wide.
