...
export function NeedsRevisionSection() {
  const colors = useColors();
  const router = useRouter();
  const rows = useQuery(api.prospects.listByState as any, { state: "needs_revision" }) ?? [];

  return (
    <StatusSection
      title="Needs Revision"
      count={`${rows.length} flagged`}
      dotColor={colors.status.revision}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            ...
            <th style={thStyle(colors)}>Revision note</th>
            <th style={thStyle(colors)}>Asked at</th>
