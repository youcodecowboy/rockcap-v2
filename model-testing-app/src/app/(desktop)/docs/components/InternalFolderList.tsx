...
interface InternalFolderListProps {
  selectedFolder: FolderSelection | null;
  onFolderSelect: (folder: FolderSelection | null) => void;
}

export default function InternalFolderList({
  selectedFolder,
  onFolderSelect,
}: InternalFolderListProps) {
  ...
  // Queries
  const folders = useQuery(api.internalFolders.list);
  const documentCounts = useQuery(api.internalFolders.getDocumentCounts);

  // Mutations
  const createFolder = useMutation(api.internalFolders.create);
  const ensureDefaultFolders = useMutation(api.internalFolders.ensureDefaultFolders);
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-4">
        <div
          ...
          style={{ borderBottom: `2px solid ${colors.accent.blue}` }}
        />
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <EmptyState
          ...
          title="No internal folders yet"
          body="Create default folders to get started"
          action={
          ...
          <div className="space-y-1">
            {folders.map((folder) => {
              ...
              return (
                ...
                >
                  {isSelected ? (
                    <FolderOpen className="w-4 h-4 flex-shrink-0" style={{ color: colors.accent.blue }} />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium truncate flex items-center gap-1.5">
                      {folder.name}
                      {folder.isCustom && (
                        <Sparkles className="w-3 h-3" style={{ color: colors.accent.purple }} />
                      )}
                    </div>
                    {folder.description && (
        ...
        <div className="space-y-4">
          <Field label="Folder Name">
            <Input
              ...
              placeholder="e.g., Contracts, Research, Reports"
            />
          </Field>
