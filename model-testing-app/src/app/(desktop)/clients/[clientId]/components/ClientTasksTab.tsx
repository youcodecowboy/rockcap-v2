...
export default function ClientTasksTab({
  ...
  // Query projects for this client (for task assignment)
  // @ts-ignore - Convex type instantiation issue
  const allProjects = useQuery(api.projects.list, { clientId }) || [];
  ...
  }, [createTask, clientId, newTaskTitle]);
            ...
            <div style={{ padding: 16, borderBottom: `1px solid ${colors.border.default}` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {hasActiveFilters && (
                    <StatusPill label={`${filteredTasks.length}`} tone={colors.accent.blue} />
                  )}
                </div>
              </div>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: selected ? `${colors.accent.blue}20` : 'transparent',
                      border: 'none',
                      cursor: 'pointer',
                    }}
                  >
        ...
        {selectedTaskId && selectedTask ? (
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {isEditing ? (
                  <>
                    <Button variant="primary" accent={colors.entityTypes.client} size="sm" onClick={handleSaveTask}>
                      <Save size={14} />
                      Save
                    </Button>
                  ...
                  <>
                    ...
                    <IconButton label="Flag for Review" onClick={() => setFlagModalOpen(true)}>
                      <Flag size={14} color={colors.accent.yellow} />
                    </IconButton>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              {/* Project Assignment */}
              {isEditing && clientProjects.length > 0 && (
                <Field label="Project">
                  <Select
                    value={editedTask?.projectId || 'none'}
                    onChange={(e) => setEditedTask({ ...editedTask, projectId: e.target.value === 'none' ? undefined : e.target.value })}
                  >
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
            <EmptyState
              ...
              body={
                tasks.length === 0
                  ? `Create tasks to track work items related to ${clientName}. Tasks can have due dates, priorities, and notes.`
                  : 'Select a task from the sidebar to view and edit it, or create a new task.'
              }
              action={
                <Button variant="primary" accent={colors.entityTypes.client} onClick={() => setIsCreating(true)}>
                  <Plus size={14} />
                  New Task
                </Button>
              }
    ...
    <div style={{ position: 'relative' }}>
      <div
        ...
        style={{
          ...
          borderLeft: isSelected ? `3px solid ${colors.accent.blue}` : '3px solid transparent',
