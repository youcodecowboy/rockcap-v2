...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const pipelines = await fetchQuery(api.hubspotSync.getPipelineName, { pipelineId: '' });
    ...
    for (const deal of deals) {
      try {
        ...
        const result = await fetchMutation(api.hubspotSync.syncDealToDealsTable, dealData);
