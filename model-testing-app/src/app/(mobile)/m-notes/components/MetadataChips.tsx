...
export default function MetadataChips({ noteId, note, onSave }: MetadataChipsProps) {
  ...
  const projects = useQuery(
    ...
    note?.clientId ? { clientId: note.clientId as Id<'clients'> } : 'skip',
  );
  ...
  const projectName = useMemo(() => {
    ...
    return projects.find((p: any) => p._id === note.projectId)?.name ?? 'Unknown';
  ...
  const handleSelectProject = async (projectId: string) => {
    await updateNote({ id: noteId as Id<'notes'>, projectId: projectId as Id<'projects'> });
  ...
  return (
    <>
      ...
      <div className="flex gap-1.5 overflow-x-auto px-[var(--m-page-px)] py-2 border-b border-[var(--m-border-subtle)] scrollbar-none flex-shrink-0">
        ...
        {/* Project chip — only when client is set */}
        {note?.clientId && (
