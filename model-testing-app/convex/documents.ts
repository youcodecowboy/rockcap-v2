...
// ── v1.3 Sprint G: persist a skill-generated document ──
//
// For skills that PRODUCE documents — lender brief packages
// (terms-package-build), IC papers (ic-paper-drafter), terms comparison
// memos (terms-comparison). These don't originate from a file upload; the
// skill generates content (typically markdown or text) and needs a documents
// row so the document surfaces in document.listByClient / project + can be
// attached to outreach drafts via outreach.draftToLender.
//
// Storage strategy: content stored inline as the document's `summary` field
// for now (markdown / text content). fileStorageId left unset. A future
// commit can wire markdown→PDF conversion via a separate action that
// uploads to Convex storage and patches fileStorageId.

export const createFromGeneration = mutation({
  args: {
    clientId: v.optional(v.id("clients")),
    projectId: v.optional(v.id("projects")),
    fileName: v.string(),            // e.g., "Comberton — Lender Brief Package.md"
    fileTypeDetected: v.string(),    // e.g., "Lender Brief Package", "IC Paper", "Terms Comparison Memo"
    category: v.string(),            // e.g., "Lender outreach", "Credit submission"
    summary: v.string(),             // the generated content (markdown or text)
    reasoning: v.optional(v.string()), // 1-2 sentence operator-facing explanation
  ...
  handler: async (ctx, args) => {
    ...
    const documentId = await ctx.db.insert("documents", {
      // No fileStorageId — content lives inline in `summary` until
      // a markdown→PDF / DOCX export step runs separately.
      fileName: args.fileName,
      fileSize: args.summary.length,                // byte-count proxy
      fileType: "text/markdown",
      uploadedAt: now,
      summary: args.summary,
      fileTypeDetected: args.fileTypeDetected,
      category: args.category,
      reasoning: args.reasoning ?? "Generated by skill; review summary content for the artefact body.",
      confidence: 1.0,                              // generated content; not classified
      tokensUsed: 0,
      clientId: args.clientId,
      clientName,
      projectId: args.projectId,
