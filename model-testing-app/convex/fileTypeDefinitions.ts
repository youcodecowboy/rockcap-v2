...
function getLatestDefinitions() {
  return [
    ...
    },
    {
      ...
      category: 'Appraisals',
      keywords: ['land appraisal', 'development budget', 'appraisal summary', 'gross margin', 'paid to date', 'cost to complete', 'timesheet', 'final budget for legals'],
      description: 'The developer\'s (client\'s) own multi-tab land appraisal / development budget workbook, mixing scheme economics with operational actuals. Developer-ops DNA throughout: Timesheet/Invoice tabs, "Approved (Sign & Date)" blocks with housebuilder directorate titles, trade-level build-cost matrices, PAID TO DATE / CTC actuals, profit as Gross Profit / Gross Margin % / Return on Sales, and finance as one lump "Total Finance Cost" line with no facility mechanics.',
      identificationRules: ['"Appraisal Summary" computing Gross Profit / Gross Margin % / Return on Sales (developer metrics, NOT Profit on Cost %)', '"Approved (Sign & Date)" block with housebuilder directorate titles', 'Trade-level housebuild cost matrix (GROUNDWORKS/BRICKLAYING… × house types £/ft2)', 'Operational actuals: Timesheet/Invoice tabs, PAID TO DATE / CTC columns', 'Shared numbers with RockCap models do NOT distinguish producer — the DNA does'],
      categoryRules: 'Client Land Appraisals should be categorized under "Appraisals".',
    ...
    },
    {
      ...
      category: 'Appraisals',
      keywords: ['lender dashboard', 'ltgdv', 'lender irr', 'money multiple', 'peak drawn loan', 'sonia', 'input checks', 'result checks', 'rockcap', 'profit on cost'],
      description: 'RockCap\'s own debt-structured appraisal model built from the client\'s numbers — the INTERNAL macro-enabled .xlsm source-of-truth workbook or its EXTERNAL lender-facing .xlsx export (values-flavoured cut with a lender-branded dashboard). Debt-structuring DNA: "Lender Dashboard - <lender>" tabs, LTGDV/LTC/Lender IRR/Money Multiple/Peak Drawn Loan/SONIA vocabulary, Input+Result Checks audit panels, Working Cells keys, monthly drawdown/repayment cashflow, Profit on Cost %, rockcap.uk URL cells. Files to Rockcap Appraisals whatever the audience.',
      identificationRules: ['Tab named "Lender Dashboard - <lender>" with Project View / Lender View panes', 'Debt-metric vocabulary: LTGDV, LTC, Lender IRR, Money Multiple, Peak Drawn Loan, margin over SONIA', 'Input Checks + Result Checks audit panel ("Fully Funded?", "Cashflow Signs Test" → OK)', 'The literal "RockCap" token in an underscore-delimited filename is by itself decisive', 'INTERNAL .xlsm may be unparseable — classify on mime + size + filename tokens'],
      categoryRules: 'RockCap Appraisal Models should be categorized under "Appraisals".',
    ...
    },
    {
      ...
      category: 'Plans',
      keywords: ['architect', 'drawing pack', 'sketch', 'concept issue', 'draft for comment', 'revision', 'title block', 'site plan markup', 'if in doubt ask'],
      description: 'Third-party architect drawing sets and annotated site plans — CAD-derived PDFs describing the physical scheme (site layout, unit sketch plans), sometimes marked up by a consultant. Extracted text is fragmentary labels only (plot numbers, room labels, all-caps tokens, no verbs). Drawing-register apparatus is decisive: architect title blocks, job-number-prefixed drawing numbers, scales, revision tables, status stamps. A leading bare numeric filename token is the architect\'s job number — never a valuer\'s reference.',
      identificationRules: ['Fragmentary label-only extracted text — no sentence structure (plot numbers, room labels, "PUBLIC FOOTPATH")', 'Drawing-register apparatus: title blocks, drawing numbers, scales "1:500 @A1", revision tables, "IF IN DOUBT, ASK."', 'Leading bare numeric filename token = third-party job number (the 4868 trap — NOT a valuation)', '"ACCOMMODATION SCHEDULE" as a drawing legend (areas in m², no pricing) is NOT the spreadsheet class'],
      categoryRules: 'Architect Drawing Packs should be categorized under "Plans".',
    ...
    },
    {
      ...
      category: 'Professional Reports',
      keywords: ['hereby permits', 'town and country planning act', 'planning permission', 'outline planning', 'reserved matters', 'decision notice', 'conditions', 'reasons'],
      description: 'The Local Planning Authority\'s statutory decision notice granting (outline) planning permission, with conditions and reasons. Operative clause "HEREBY PERMITS the development described below subject to the conditions stated" under a "TOWN AND COUNTRY PLANNING ACT, 1990" citation, with a planning reference, Applicant/Agent/Dated block, and numbered Conditions mirrored by numbered Reasons. "Reserved matters" marks an outline permission. A dated statutory record — no versions.',
      identificationRules: ['Operative clause "HEREBY PERMITS …"', '"TOWN AND COUNTRY PLANNING ACT, 1990" citation + planning reference (S.NN/NNN pattern)', 'Numbered Conditions with mirrored numbered Reasons', '"Reserved matters" = outline-specific token'],
      categoryRules: 'Planning decision notices should be categorized under "Professional Reports".',
      targetFolderKey: 'modelling_info',
      targetLevel: 'project' as const,
    },
    {
      ...
      category: 'Professional Reports',
      keywords: ['s106', 'section 106', 'discharge', 'variation', 'planning obligation', 'hereby agrees to discharge', 'modification and discharge', '106R'],
      description: 'The council\'s statutory decision varying or discharging a Section 106 planning obligation. Cites "The Town and Country Planning (Modification and Discharge of Planning Obligations) Regulations 1992", carries the operative clause "HEREBY AGREES TO DISCHARGE the Section 106 Agreement", a planning reference with the /106R suffix, and NPPF obligation tests as grounds. Distinct from the original S106 deed (a long legal agreement with parties/recitals/covenants).',
      identificationRules: ['"Modification and Discharge of Planning Obligations Regulations 1992" citation', 'Operative clause "HEREBY AGREES TO DISCHARGE the Section 106 Agreement"', 'Planning ref suffix "…/106R"', 'NPPF planning-obligation tests quoted as grounds'],
      categoryRules: 'S106 decisions should be categorized under "Professional Reports".',
      targetFolderKey: 'modelling_info',
      targetLevel: 'project' as const,
    },
    {
      ...
      category: 'Professional Reports',
      keywords: ['commencement', 'extant', 'lawful commencement', 'planning officer', 'our ref', 'your ref'],
      description: 'Local Planning Authority officer correspondence confirming that works constitute lawful COMMENCEMENT of the approved development, keeping the planning permission EXTANT — a key underwriting fact. Letter form (Dear/Yours sincerely from a planning officer) on council letterhead with an Our Ref/Your Ref planning-reference pair, cross-referencing the parent permission. Not a decision form: no HEREBY clause, no Conditions/Reasons.',
      identificationRules: ['"constitutes a commencement of the overall approved development" + "the planning permissions will remain extant"', 'Council letterhead + Our Ref/Your Ref planning reference pair', 'Letter apparatus from a planning officer — not a decision form', 'Cross-reference to the parent outline permission'],
      categoryRules: 'Commencement confirmations should be categorized under "Professional Reports".',
      targetFolderKey: 'modelling_info',
      targetLevel: 'project' as const,
    },
    {
      ...
      category: 'Professional Reports',
      keywords: ['pricing exercise', 'pricing proposal', 'does not constitute a valuation', 'estate agency role', 'guide price', 'psf', 'general assumptions'],
      description: 'An estate agent\'s sales-pricing opinion for the scheme — expressly NOT a valuation — in narrative letter-proposal or standalone schedule form. Literal "PRICING EXERCISE" heading and the disclaimer "does not constitute a valuation or appraisal and must not be construed or relied upon as such", produced "in the course of … our estate agency role, as an informal document". Housetype/Sq ft/Guide price/£PSF schedule with Private vs Affordable split. NO RICS/Red Book/Market Value language anywhere — the text-body disclaimer outranks a filename saying "Valuation".',
      identificationRules: ['Literal heading "PRICING EXERCISE" / "Pricing proposal for…"', 'Disclaimer "does not constitute a valuation or appraisal…" — outranks filename tokens (the Red Book rule)', 'Housetype/No./Sq ft/Guide price/£PSF schedule with Private vs Affordable split', 'NO RICS / Red Book / Market Value / valuer-reliance language anywhere'],
      categoryRules: 'Agent pricing reports should be categorized under "Professional Reports".',
      targetFolderKey: 'modelling_info',
      targetLevel: 'project' as const,
    },
    // KYC
    ...
    // LOAN TERMS
    {
      ...
      category: 'Loan Terms',
      keywords: ['indicative terms', 'heads of terms', 'without commitment', 'subject to credit approval', 'broker fee', 'introducer fee', 'ltgdv', 'in principle'],
      description: 'A single lender\'s indicative/heads-of-terms quote for the deal, in whatever medium the lender sent it (formal PDF term sheet or screenshot of emailed terms). Hedge boilerplate ("Indicative Terms", "Without Commitment", "Subject to Credit Approval"), lender first-person voice, and the broker appearing as a FEE LINE (not an author) mark it lender-produced. Exactly one lender\'s terms — two-plus lenders side-by-side is a RockCap-produced comparison (terms analysis).',
      identificationRules: ['Hedge boilerplate: "INDICATIVE LOAN TERMS", "WITHOUT COMMITMENT AND SUBJECT TO CREDIT APPROVAL", "Subject to our normal lending criteria"', 'Broker as a fee line ("Broker Fee", "Introducer Fee", "RockCap\'s introductory fee") — cannot be BY the broker', 'Lender first-person voice / letterhead / regulatory footer / proprietary rate construct', 'Exactly one lender\'s terms, no comparison grid; borrower named as JV/prospect entity or "SPV TBC"'],
    ...
    },
    {
      ...
      category: 'Loan Terms',
      keywords: ['credit backed terms', 'credit approved', 'credit committee approved', 'approved terms', 'committed terms', 'resolved spv', 'credit-stage terms'],
      description: 'The chosen lender\'s refreshed/credit-backed terms issued at or after credit submission — often on the SAME "Indicative Terms" template as sourcing-window quotes. Stage is decided by content, not the heading: the borrower is named as the resolved borrowing SPV (not the JV/prospect or "SPV TBC"), the date post-dates the credit checklist cycle, and the numbers are fully tranched and reconciled.',
      identificationRules: ['Borrower named as the RESOLVED borrowing SPV (strongest credit-stage signal — the template heading may still say "Indicative Terms")', 'Date post-dates the credit submission/checklist cycle', 'Fully tranched and reconciled availability breakdown (incl. broker fee, S106/CIL, interest allowance)', 'Or: explicit "Credit Backed" / "Credit Approved" / credit committee approval statement'],
    ...
    },
    {
      ...
      category: 'Loan Terms',
      keywords: ['lender comparison', 'comparison sheet', 'total cost of debt', 'arrangement fee', 'exit fee', 'like for like', 'margin'],
      description: 'RockCap\'s like-for-like multi-lender comparison workbook — scheme constants held fixed while lender pricing varies, run through the model. Three or more named lenders arrayed as columns against identical criteria, with a Total Cost of Debt ranking row. INTERNAL .xlsm working model or EXTERNAL .xlsx client cut.',
      identificationRules: ['≥3 named lenders as columns against identical criteria (Margin / Arrangement Fee / Exit Fee / Gross Loan / Term)', 'Scheme constants identical in every lender column — the like-for-like tell', '"Total Cost of Debt (incl fees)" ranking row differing by lender', 'Multi-lender comparison TRUMPS embedded appraisal/cashflow sheets — do not demote to an appraisal type'],
      categoryRules: 'Lender Comparison Sheet should be categorized under "Loan Terms".',
    ...
    },
    {
      ...
      category: 'Loan Terms',
      keywords: ['lender comparison table', 'triage', 'ic status', 'declined', 'raw terms transcription'],
      description: 'The early scratch grid transcribing incoming term sheets raw, before terms are run through the model like-for-like. Transposed vs the modelled comparison workbooks (lenders as ROWS, criteria as columns), with an "IC Status" column (Indicative / Declined) and free-text raw transcription notes. Typically an un-versioned, pre-convention filename.',
      identificationRules: ['Lenders as ROWS, criteria as columns (transposed vs the modelled comparison)', '"IC Status" column with values like "Indicative" / "Declined"', 'Raw term-sheet transcription notes, no modelled IRR/proceeds rows', 'Un-versioned bare "<Project>_<DocType>" filename — early scratch tell'],
      categoryRules: 'Lender Comparison Table should be categorized under "Loan Terms".',
      targetFolderKey: 'terms_analysis',
      targetLevel: 'project' as const,
    },
    {
      ...
      category: 'Loan Terms',
      keywords: ['lender analysis', 'analysis and recommendation', 'canvassed', 'confidential', 'lender-by-lender assessment'],
      description: 'RockCap\'s narrative analysis-and-recommendation memo to the client, comparing the canvassed lender panel and recommending one. Title block "…Analysis & Recommendation — Prepared by RockCap Ltd … Confidential"; one-row-per-lender comparison table; candid lender-by-lender assessments; a named recommendation with negotiated deltas. External (client-facing) despite full lender candour.',
      identificationRules: ['Title block "Analysis & Recommendation — Prepared by RockCap Ltd … Confidential"', '"RockCap has canvassed the development finance market on behalf of …" + cross-reference to the accompanying Excel workbook', 'Lender-by-Lender Assessment sections with candid views', 'Named recommendation with negotiated deltas and an ask to review and revert'],
      categoryRules: 'Lender Analysis Note should be categorized under "Loan Terms".',
      targetFolderKey: 'terms_analysis',
      targetLevel: 'project' as const,
    },
    // LEGAL DOCUMENTS
    ...
    // PROJECT DOCUMENTS
    {
      ...
      keywords: ['accommodation schedule', 'unit schedule', 'unit mix', 'plot', 'nia', 'tenure', 'gdv', 'pricing tbc'],
      description: 'The SUBJECT-scheme unit grid — the plot-by-plot GDV build-up (rows keyed by small sequential Plot integers, tenure-banded, priced) that the comparable evidence must support. NIA (Sq Ft) areas, pricing columns named after valuers/agents (forward-looking targets, no transaction dates), tenure bands with subtotals summing to a scheme GDV. Decisive vs Comparable Schedule: plot-number rows (subject) vs postal-address rows (external evidence); no date column vs historic sold dates; no source citations vs Evidence Links.',
      identificationRules: ['Row key is a bare Plot number (small sequential integers), not addresses', 'Tenure banding rows ("Affordable (n units)" / "Private (n units)") with a Total Scheme GDV row', 'NO date column, NO Evidence Link, NO source citations — prices are targets/opinions', 'Area column headed "NIA (Sq Ft)"'],
    ...
    },
    {
      ...
      category: 'Project Documents',
      keywords: ['comparable schedule', 'appendix a', 'master comparable schedule', 'comparable evidence', 'tier', 'rightmove', 'asking', 'evidence link'],
      description: 'RockCap\'s lender-facing master schedule of comparable transaction evidence for OTHER properties, tier-banded and source-cited, compiled to support the subject\'s GDV (the credit pack\'s "Appendix A" deliverable). Rows keyed by postal addresses of other properties/schemes; Date column with historic sold dates or "ASKING" flags; Evidence Link / source column (Rightmove, Realyse, EPC); tier banding by scheme/distance; self-declaring header "Comparable evidence for lender credit pack". Order series members by the DATE token, not the version token.',
      identificationRules: ['Rows keyed by postal addresses of OTHER properties, other developers named', 'Date column with historic sold dates and/or "ASKING" flags with exclusion rules', 'Evidence Link / source column (Rightmove URLs, "RM Postcode", Realyse, EPC)', 'Tier banding by scheme/distance; notes compare each comp TO the subject in third person'],
      categoryRules: 'Comparable Schedules should be categorized under "Project Documents".',
    ...
    },
    {
      ...
      category: 'Project Documents',
      keywords: ['credit checklist', 'in lender pack', 'now provided', 'outstanding', 'borrowing entity', 'golden brick', 'guarantor'],
      description: 'RockCap\'s internal tracker of a specific lender\'s credit information requests, with tri-state status and dated evidence provenance. Unique marker: the status vocabulary "In Lender Pack | Now Provided | Outstanding" with a legend, under a "Lender: <name> / Broker: RockCap Ltd" header. Requirement rows phrased as lender asks; Notes column of dated confirmations naming individuals on both sides; section rows mirroring a credit paper (CLIENT / SITE AND PLANNING / CONSTRUCTION / SALES & MARKETING). The embedded lender token in the filename is the tell — credit docs are lender-specific.',
      identificationRules: ['Tri-state status "In Lender Pack | Now Provided | Outstanding" with a legend', '"Lender: <name> / Broker: RockCap Ltd" header block', 'Requirement rows phrased as lender asks with dated evidence-trail notes', 'Successive versions are pure status migration (Outstanding → Now Provided)'],
      categoryRules: 'Credit Checklists should be categorized under "Project Documents".',
      targetFolderKey: 'credit',
      targetLevel: 'project' as const,
    },
    {
    ...
    },
    {
      ...
      category: 'Communications',
      keywords: ['lender brief', 'rockcap is pleased to present', 'relationship manager', 'borrower entity', 'funding requirement', 'note'],
      description: 'RockCap\'s ~1-page outbound deal memo pitching the financing opportunity to prospective lenders, in the RockCap "Note" house template. Subject line begins "Lender Brief:"; body opens "RockCap is pleased to present…" (pitch voice); labelled ask fields ("Borrower Entity:", "Funding Requirement:"); numbered section skeleton; the first body line embeds its own version name-stamp. Lender-agnostic (briefs precede lender selection). Audience is EXTERNAL by content even for filing copies whose FILENAME says INTERNAL — the body stamp + register win.',
      identificationRules: ['Subject line begins "Lender Brief:"', 'Opening "RockCap is pleased to present …" — promotional pitch voice', '"Relationship Manager:" preamble + numbered 4-section skeleton', 'First body line embeds its own version name-stamp; body stamp beats filename audience token'],
      categoryRules: 'Lender Brief Notes should be categorized under "Communications".',
    ...
    },
    {
      ...
      category: 'Communications',
      keywords: ['initial call', 'call note', 'user note', 'cite', 'strategic actions', 'relationship manager'],
      description: 'RockCap\'s internal, machine-assisted record of a client call at deal origination, in the Note house template, with cited provenance and owner-assigned actions. Decisive marker: "[User Note, cite: N]" citation artifacts on nearly every line, plus a "Strategic Actions" section assigning tasks to initials and a "Filing:" instruction. Names multiple candidate lenders for the same need (pre-selection talk) and carries client-sensitive candour. A point-in-time record — may contain early facts later superseded.',
      identificationRules: ['"[User Note, cite: N]" citation artifacts — decisive internal call-note marker', '"Strategic Actions" section assigning tasks to initials + "Filing:" instruction', 'Multiple candidate lenders named for the same need — never in external docs', 'Note-template skeleton: name-stamp + Subject/Date/Relationship Manager + numbered sections'],
      categoryRules: 'Initial Call Notes should be categorized under "Communications".',
      targetFolderKey: 'notes',
      targetLevel: 'project' as const,
    },
    // WARRANTIES
