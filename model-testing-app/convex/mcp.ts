...
const TOOLS: McpTool[] = [
  ...
  },
  {
    ...
    description:
      "THE prospecting inbox — the org-wide, client-linked view over BOTH mail directions in one feed, newest first. Inbound rows are replyEvents with a linkedClientId (contact-email match at ingest); outbound rows are gmail email touchpoints with a relatedClientId — in-app approved sends AND manual Gmail sends (the SENT poller captures those automatically since 2026-07-17, deduped against in-app sends by Gmail message id). Distinct from the operator's private /inbox: this surface only ever shows business correspondence. Each row: kind (inbound/outbound), occurredAt, subject, 160-char snippet (never full bodies — drill in with reply.get / touchpoint reads), clientId/clientName/pipelineStage, contact, counterpartyEmail, classifiedIntent + resolvedAt (inbound), hasAttachments. Filters: stage (one of the 5 manual pipeline stages), direction, includeNonProspects (default FALSE — prospects only; pass true to include active clients), windowDays (default 45, max 120 — it is a recency surface, not an archive). Use for 'what mail moved with prospects this week', per-stage triage sweeps, and as the evidence read behind KPI questions — for the counts themselves prefer outreach.prospectingKpis.",
    inputSchema: {
      type: "object",
      properties: {
        stage: {
