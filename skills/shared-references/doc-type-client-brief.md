...
## Composing & rendering
Build a `ClientBriefData` object (`model-testing-app/src/lib/docgen/types.ts`) and render with `renderDocument({ layout: "client-brief", briefData, title, formats })`. Section `bodyHtml` is injected raw — emit clean semantic HTML (`<p>`, `<table>` with `class="num"` on numeric cells, `class="caption"` on source/footnote lines). Shell fields (title, meta, key facts, sign-off) are escaped by the layout.
