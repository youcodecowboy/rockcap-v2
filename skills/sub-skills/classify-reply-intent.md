...
Sub-skill: classify an inbound reply into one of seven intent buckets so the cadence-fire reply handler can dispatch to the right downstream destination.
...
```json
{
  "intent": "book_meeting" | "defer_long_term" | "not_interested" | "info_question" | "positive" | "out_of_office" | "unknown",
  "confidence": 0.0 - 1.0,
...
| `positive` | A meaningful, forward-moving reply that is neither a specific meeting request nor a specific question. Phrases: "sounds interesting, tell me more", "good timing", "this could be relevant for us", "keen to learn more". Positive engagement intent — warrants a drafted reply to keep momentum — but stops short of proposing a meeting or asking a concrete question. |
