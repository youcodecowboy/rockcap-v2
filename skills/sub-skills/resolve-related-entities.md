...
## Output: StructureGraph (for the corporate-structure skill)

In addition to `relatedCompaniesHouseNumbers` and the prose `borrower.related_entities` knowledge item, emit a `StructureGraph` (shape in `model-testing-app/src/lib/structure/types.ts`): one node per entity/person, one edge per ownership/directorship/charge/PSC relation. Tag every edge with its `evidence` (the CH filing), a `confidence` (`hard` = filed declaration; `soft` = inferred/band-only), and `flags` where they apply (`director-not-owner`, `brand-not-borrower`, `band-only`, `ceased`, `dissolved`, `unverified`). This graph feeds `gradeStructure` (the verdict) and `buildStructureChartSvg` (the chart).
