...
### `outreach.*` — Email draft staging + triage + metrics (5)
...
### `caseStudy.*` — Deal Book case-study index (5)

> Ships with PR #66 — live from the first Convex deploy of a tree containing it (deploys are whole-tree snapshots; deploying a branch without this code removes these tools again).

| Tool | Purpose |
|---|---|
| `caseStudy.deriveDrafts` | Scan completed projects and create draft case-study entries (idempotent). Use to seed the index from existing deal history. |
| `caseStudy.confirm` | Operator confirms or edits a draft — sets sector, dealType, region, headline, and referenceable flag. Must be called before a case study is hook-eligible. |
| `caseStudy.matchForProspect({sector, region})` | Anonymised track-record match for hook-ladder rung 9. Returns only operator-confirmed, `referenceable` entries projected to `{sector, region, sizeBand, headline}` — no client names. |
| `caseStudy.list` | List case studies (joined with project data) for operator review and ops. |
| `caseStudy.setReferenceable` | Toggle a case study's hook eligibility (referenceable true/false) without a full confirm pass. |

### `dealBook.*` — Deal Book portfolio aggregates (1)

> Ships with PR #66 — same deploy caveat as `caseStudy.*` above.

| Tool | Purpose |
|---|---|
| `dealBook.stats` | Portfolio-level aggregates: open / closed / lost counts + closed-deal windows (30 / 90 / 180 / 365 days). Use for high-level pipeline reporting. |
