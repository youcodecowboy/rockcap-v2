...
The complete, canonical list of MCP tools exposed by the RockCap Convex backend (`https://incredible-kudu-562.convex.site/mcp`). 188 tools across 31 domains. Latest pass (prospecting inbox + full-duplex tracking, 2026-07-17): adds `outreach.prospectingInbox` (org-wide client-linked mail, BOTH directions, stage-filterable — the read behind the new /inbox Prospects box) + `outreach.prospectingKpis` (per-stage sent/received/meetings-held/meetings-upcoming). Substrate shipped in the same pass: the Gmail poller now ALSO captures SENT mail (contact-matched → outbound gmail touchpoints, deduped against in-app sends by Gmail message id; 45d history backfilled) — manual Gmail sends are tracked automatically, `touchpoint.logManualSend` is now only for non-Gmail channels; and Google Calendar sync got the attendee-shape fix (events with attendees were silently DROPPED by schema validation since the beginning) + an attendee→contact→prospect matcher stamping events.clientId/linkedContactIds (profile Meetings tab shows matched calendar events; NOTE: both calendar connections currently need re-auth at /calendar). (Count reconciled against the live meta.listTools 2026-07-16 — added the previously-missing rows for `intel.revalidate` + `prospect.promoteStage` + `prospect.setQualSubStage`, shipped in earlier passes without their catalogue rows here.) Latest pass (Gmail attachments → Drive, 2026-07-16): adds `reply.listAttachments` (list an inbound email's attachments LIVE from Gmail — works via replyEventId or a raw Gmail/RFC822 message id, including rows ingested before capture existed) + `drive.saveEmailAttachment` (stage copying an attachment out of the mailbox owner's Gmail into a mirrored Drive folder — the FOURTH `drive_write` op, same operator-approval + `/settings/drive` kill-switch double gate; optional `importToLibrary` also files it as a metadata-first document). Same pass: the Gmail inbound poller now captures attachment METADATA (`replyEvents.attachments` — filename/mimeType/size/partId/inline flag — plus `gmailApiId`), so reply feeds show which inbound mail carries files; bytes stay in Gmail until an approved save copies them. Prior pass (metrics substrate Phase 2, 2026-07-15): adds `outreach.metrics` — the OUTCOME layer over the state-checking triage: sends / substantive replies / contact-level response rate / touchesPerEarnedReply / by-template table (replies attributed to the latest fired touch; `templateKey` stamped by the drafting skills flows dispatcher→send payload). Same pass: first-edit snapshots (`approvals.originalDraftPayload`, `cadences.originalPreDraftedTouch`) make draft-vs-sent change-detection an exact diff. Prior pass (cold-reachout Phase 1, 2026-07-15): adds `deal.listByStage` — the SELECTION read for the `/cold-reachout N` action command: mirrored HubSpot deals in a pipeline stage joined to their app prospects with explicit dedupe flags (`alreadyWorked` = send evidence, skip in cold selection; `contactWithEmail` = can it actually send). Pairs with the new `cold-reachout` skill in RockCap-MCP (gate 1 approve-list → prospect-intel → outreach-draft w/ templateKey stamped → jittered Friday/Saturday send-window scheduling → gate 2 approvePackageBatch → proposed stage moves). Prior pass (Outreach triage backbone, 2026-07-14): adds `outreach.triageQueue` (THE cross-prospect triage read — every open outreach action in one call: pending cadence packages, needs-contact holds, reply drafts, other pending approvals, failed sends, unrouted + dead-end replies, silently-stalled cadences (intel_hold / 3-strike auto-deactivation / paused), needs-action flags + stale intel; ALWAYS the first call of an /outreach session) + `cadence.listUpcoming` (the OUTBOX: every active touch due in the next N days with an honest per-touch fireStatus — scheduled/due_now vs paused/blocked_package_pending/blocked_no_contact_email — the 'whether and when will cadences fire' answer) + `approval.approveBatch` (approve ≤50 reviewed approvals in one call, per-item no-op-safe; itemise to the operator first) + `cadence.approvePackageBatch` (approve ≤25 packages with ONE dispatcher kick at the end; per-item no-contact-guard failures reported, not thrown). Also ships GET `/triage-digest` on the Convex deployment — the compact stage-workspace session digest the RockCap-MCP SessionStart hook injects, authenticated by the SAME per-user MCP bearer token as `/mcp` (no extra credential; the hook derives URL + token from `.mcp.json`). Same pass: `approval.retry` (re-queue an execution_failed send — the recovery for triageQueue's failedSends) + `cadence.reactivate` (clear a stalled cadence's intel-hold / failure-deactivation / pause in one call; refuses deliberate stops). Backlog-reset sub-pass (same day): `reply.resolveBatch` (mark ≤100 replies handled — leaves every queue, keeps history + handled-by attribution in the app), `cadence.denyPackageBatch` (deny ≤25 stale packages), `approval.rejectBatch` (discard ≤50 pending drafts), `client.dismissNeedsActionFlag` — the four primitives that let /outreach drive the triage queue to ZERO, built for reconciling system state with outreach that happened manually via Gmail outside the pipeline. Reconciliation writes (same day): `cadence.adoptManualSend` (AUTOFIT a drafted package onto a manual send: touch 1 marked fired-externally at the real date, follow-ups refit from it, touchpoint + lastOutreachSendAt + state advance — the no-double-email way to CONTINUE a sequence the operator started by hand) + `touchpoint.logManualSend` (backfill ≤50 manual sends as outbound touchpoints when no package continues). Prior pass (Lender profile write + document trail, 2026-07-11): adds `facilities.updateTerms` (update amount/rate/maturity — operator value holds until newer document evidence rematerializes the row; operator-created rows never rebuilt) + `facilities.create` (manually add a facility to a lender's book — operator lane for deals the documents haven't evidenced; flagged createdFrom:'operator', minter-compatible dedupeKey so a later document-evidenced mint lands on the same row) + `facilities.setStatus` (operator override for facility lifecycle status — ANY transition incl. downgrades; the pipeline still never downgrades on its own) + `lender.getDocuments` (the lender's document evidence trail: every document evidencing the lender, federated from the lender row's sourceDocumentIds + lender knowledge atoms + facility-book atoms + appetite sources, each with summary/scope/atomCount/via lanes — powers the Lenders-tab Documents view). Together with the existing `lender.recordAppetite` + `contact.create({clientId})`, Claude can now build out a lender profile end to end: roster (lender.create upsert) → facilities → appetite → people → documents. Prior pass (Lender enrichment, 2026-07-11): adds `companies.getOfficers({companyNumber})` — read back the officers + PSCs of an already-synced CH company (names, roles, appointed/resigned + isActive, appointmentsLink; active-first). Closes the loop that blocked contact discovery: `companies.syncCompaniesHouse` persisted officers but returned only counts, and `apollo.findEmail` can only enrich a NAMED person — so there was no MCP path from "lender row" to "named people to enrich". Seed for the lender-intel ENRICH gauntlet (CH match → sync → getOfficers → apollo.findEmail per name → contact.create) and reusable by prospect-intel step 8. Prior pass (Lender-DB hardening, 2026-07-09): adds `lender.merge` (collapse a duplicate lender row into a canonical survivor — repoints atoms/facilities/appetite/chunks + CRM refs + clientRoles, unions row fields, soft-deletes the from-row; `dryRun` preview) + `facilities.audit` (find/fix FRAGMENTED facility rows — the multiple rows free-text tranche descriptors used to mint per quote revision; dry-run report or `execute:true` merge). Same pass: `lender.create` mode 3 is now an UPSERT (conservative normalized-name dedup across name/companyName/aliases before insert; new `aliases` + `sourceDocumentIds` args), and facilities gain a CLOSED tranche enum (senior/mezzanine/bridge/equity — free text → `single`, so quote revisions hit ONE row) + indicative/live status stamped from the triggering doc type at mint time. Prior pass (Knowledge Layer — atlas snapshot, 2026-07-08): adds `graph.overview` — the ORG-WIDE knowledge-graph snapshot behind the atlas board: EVERY entity and edge in one call (all clients flagged lender/borrower/developer + active/prospect, all projects, plus every contact/company/facility/candidate with ≥1 atom or structural edge), federating live ATOM edges (active+contested only; superseded/retired skipped) with NATIVE structural edges via the same synthesizers `graph.expandEntity` uses, deduped per (from, to, predicate) — atom wins over its native mirror (`corroborated:true`), a contested duplicate is never hidden by a calmer one. Nodes carry atomCount/contestedCount/degree; response bounded (maxNodes 2500 / maxEdges 6000, lowest-degree contact/company/candidate leaves dropped first, `counts.truncated` flagged). Use for whole-book questions (which lenders fan across clients, where contested hotspots sit), then drill with `graph.expandEntity`. Prior pass (Knowledge Layer — chunk retrieval lane, 2026-07-08): `atoms.search` now returns TWO result lanes — `results` (atoms: discrete facts with provenance, hybrid full-text + vector + RRF as before) AND `chunks` (prose passages from the `documentChunks` narrative dual index, previously write-only: same hybrid+RRF over chunk text, each hit self-describing with parent-document displayName/fileName/fileTypeDetected + locator {page/sheet/section} + chunkIndex, text trimmed to ~700 chars with `truncated:true`). `includeChunks` defaults true (set false for atoms only), `chunkLimit` default 8 / max 20; chunk retrievals log to `retrievalLog` as chunkId rows (salience-inert — usage salience stays atoms-only). Prior pass (Knowledge Layer — entity merge, 2026-07-08): adds `atoms.mergeEntities` — the operator-hygiene path to collapse a DUPLICATE existing entity (twin clients created twice via HubSpot/promotion, `(175)`-suffixed duplicate contacts, a company synced under two Companies House rows) on the KNOWLEDGE GRAPH. Re-points every atom (subject + object side) AND the denormalized scope/mirror refs — atoms.clientId/projectId, the `facilities` lender/borrower/company/project columns, documentChunks scope tags, appetiteSignals.lenderClientId — from the source id to the canonical one, routing each atom through the SAME identity machinery Phase-2b candidate resolution uses (a re-pointed atom that now duplicates a live atom MERGES with observations moved + corroboration bumped; a value clash goes CONTESTED — never a silent double). Knowledge-graph ONLY and CRM-BLIND: the CRM-table reassignment (contacts/documents/tasks/…) and the source-row soft-delete stay with `migrations/mergeDuplicateClients`, its atom-blind mirror. Writes an `auditLog` row with `{repointed, merged, contested}` + scope counts. `entityType` 'candidate' delegates to the candidate-resolution path (resolve a candidate to a real entity, pass `toType`). Prior pass (Knowledge Layer Phase 2b — entity-candidate enrichment, 2026-07-08): adds `atoms.createCandidate` / `atoms.listCandidates` / `atoms.dismissCandidate` — provisional entities for unresolvable mentions (spec §3.5), so the atomize-document repair path NEVER drops a fact: mint a candidate for an unresolved person/company, anchor the atom to it (subjectType/objectEntityType 'candidate'), and the background enrichment worker (2-hourly Convex cron, 10 candidates/tick) resolves it — companies via Companies House exact-normalized-name search + full sync (profile/charges/officers/PSCs → `companiesHouseCompanies`), people via client-scoped contact match then Apollo (conservative: requires a clientId scope hint + a returned email; new contact minted with emailSource 'apollo'). On resolution every referencing atom is re-pointed to the real entity THROUGH the identity machinery: a re-pointed atom that now duplicates an existing live atom MERGES (observations move to the survivor, corroboration bumps, duplicate superseded); a value clash goes contested. Dedup: same normalized mention reuses ONE candidate row; a resolved candidate re-encountered returns the REAL entity ref so callers anchor directly. Failed attempts cap at 3, then the candidate stays pending for operator triage (`atoms.listCandidates` → `atoms.dismissCandidate`); never auto-dismissed; infra errors (missing CH/Apollo keys) don't burn attempts. Pass `clientId` on createCandidate whenever known — it is stored as a scope hint inside contextSnippet ('client:<id>|<snippet>'; schema frozen). Prior pass (Drive wide-net auto-import + classification identity, 2026-07-07): adds `drive.setAutoImport({driveFolderId, enabled})` — a STANDING AUTHORIZATION on a folder subtree: NEW files dropped there auto-import on the poll tick that mirrors them (metadata-first document immediately, then v4 API classification at a few cents per file). The flag inherits like the project mapping (nearest ancestor-or-self with it EXPLICITLY set wins; `enabled:false` carves a subfolder out of a flagged parent) and is inert outside a client-mapped scope. Guard rails: capped at 20 auto-imports/day per flagged folder — beyond the cap files stay mirrored but UNIMPORTED, the folder is badged (`autoImportCapHit`) in the `/settings/drive` tree and on `drive.listFolders`, and cap-skipped files do NOT retro-import the next day (they are no longer 'new' to the mirror) — run `drive.importFolder` / a harness classification wave for the remainder; atomization stays harness-lane regardless. `drive.listFolders` now also returns `effectiveAutoImport`/`isExplicitAutoImport`/`autoImportCapHit`. Same pass: CLASSIFICATION IDENTITY IS NOW IMMUTABLE — both persistence lanes (API re-extraction via `driveHydration.applyExtraction` and harness `document.applyClassification`) refresh CONTENTS only (summary/textContent/documentAnalysis/extracted figures/checksum) on an already-classified document and never overwrite a real fileTypeDetected/category ('an appraisal never stops being an appraisal; edits change contents, never identity'); `document.applyClassification` returns `identityLocked:true` when it kept the original identity, and reclassification is a future explicit operator tool (`document.reclassify` — not built yet). Prior pass (Harness classification, 2026-07-07): adds `document.extractText` + `document.applyClassification` — the split-open middle of the v4 pipeline, so bulk document classification runs through Claude Code at SUBSCRIPTION cost instead of the Haiku API. `extractText` is server-side parse only (zero LLM — pdf-parse/xlsx/mammoth via the thin `/api/knowledge/extract-text` route; for a pending Drive doc it fetches the bytes and CLAIMS the mirror row so the hydration cron doesn't race the agent), the AGENT classifies, and `applyClassification` persists with the pipeline's exact semantics: server-side deterministic filing via the placement-rules table on FIRST classification only (agents never choose folders, filed docs never move), side-effect parity (KB entry create-only, meeting-job heuristics, context-cache invalidation, ingestionEvents row) and drift-aware completion of the driveFiles row (file edited mid-classification → re-arms settling, the automatic pipeline re-extracts). Cost model: the harness lane owns bulk/onboarding classification (no API spend — the operator's subscription does the classifying); the API pipeline (driveHydration cron → `/api/drive/ingest`) remains ONLY for automatic re-processing when a Drive file changes. Fused classify+atomize procedure in the `atomize-document` skill's "Full onboarding" section. Prior pass (Knowledge Layer 2a.4, 2026-07-06): adds the graph READ side (4 tools) — `atoms.search` (full-text over atom statements; the entity-resolution entry point of a graph walk — vector+RRF hybrid arrives with 2a.2) + `graph.expandEntity` (ONE federated hop: atom edges + native structural edges synthesized live from clientRoles / contacts / group SPVs / CH officers+PSC (exact-name matches only) / facility columns; when both lanes assert the same edge the atom wins with `nativeCorroboration` noted; fan-out ranked contested→confidence→recency and truncated to `limit` with full counts) + `graph.sharedNeighbors` (intersect neighborhoods — "what connects these?") + `graph.findPaths` (bounded ≤3-hop BFS, ≤5 provenance-per-hop paths). Claude is the query planner — a hop is one call, multi-hop reasoning is a sequence of calls with pruning between hops; no retrieval router. `prospect.`/`client.getDeepContext` now returns a bounded `graph` section ({atoms: 0} for knowledge-empty clients; else atom/contested counts + top-10 federated edges + facilities). Prior pass (Knowledge Layer 2a.3, 2026-07-06): adds the `atoms.*` domain (5 tools) — the harness-lane write surface for the atomic-fact knowledge graph (Spec 2). `atoms.vocabulary` (legal predicates) → `atoms.getForSubject` (existing coverage) → `atoms.createBatch` (persist ≤100 candidates through the anchored/discriminating/material gates; READ + repair the `rejected` array) → `atoms.upsertChunks` (narrative dual index) → `atoms.supersede` / `atoms.retire` (operator hygiene). The harness lane (Claude Code + `atomize-document` skill) does bulk/backfill atomization at subscription cost; the API-lane cron handles cheap incremental re-atomization of changed docs, gated by a cost wall. Prior pass (Drive project mapping, 2026-07-06): adds `drive.mapFolderToProject` — map a Drive subfolder (inside a client-mapped subtree) to an in-app project so imports from that subtree stamp `projectId`/`projectName` and file into the PROJECT folder taxonomy instead of polluting the client library. Scope-only, exactly like the client mapping (imports/extracts nothing); the project must belong to the folder's effective client (rejected otherwise). `drive.listFolders` now also returns `effectiveProjectId`/`effectiveProjectName`/`isExplicitProjectMapping`. Onboarding order: map the client folder → map project subfolders → import per project. Prior pass (Drive write-back, 2026-07-06): adds `drive.createFolder` / `drive.moveFile` / `drive.rename` — the ONLY writes the app ever makes back to Drive (organizational: create folder / move file / rename; never file contents — Drive stays the source of truth for bytes). Each call stages a PENDING `approvals` row (entityType `drive_write`); NOTHING touches Drive until the operator approves at `/approvals`. On top of the approval sits the write-back kill switch at `/settings/drive` (default off, no row = disabled) — checked at queue time (nothing staged if off) AND re-checked at execute time (an approval whose switch was flipped off while pending lands as `execution_failed`, Drive untouched). On success the executor echoes the change into the mirror immediately, so the next poll tick's change event is an idempotent no-op. Prior pass (Drive ingestion, 2026-07-06): adds the `drive.*` domain (7 tools) — the MCP surface over the one org-wide Google Drive connection (mirrored every 2 min). `drive.status` → `drive.listFolders` / `drive.listFiles` / `drive.getFile` (read the mirror) → `drive.mapFolderToClient` (scope only — imports/extracts nothing) → `drive.importFiles` / `drive.importFolder` (the purposeful act: metadata-first `documents` row now, v4 extraction within the settle window). Folder imports **dry-run first** (a cost barrier — every imported file is later extracted through the Claude v4 pipeline); the agent presents the count and re-calls with `confirm:true` only after operator approval. Prior pass (prospecting v3 action layer, 2026-06-26): adds `intel.revalidate({clientId, companyNumber?, reason?})` — the cheap mode-2 intel-freshness diff (returns still_valid | materially_changed; full `prospect-intel` is mode 1). Also threads `revalidateResult` through `skillRun.complete` (denormalised onto `clients.lastIntelResult`). Prior pass (onboard-at-stage, 2026-06-08): adds `prospect.import` — one-call onboard of an EXISTING, manually-worked prospect directly at any pipeline stage (back-fills companies with prior outreach without the top-of-funnel `prospect-intel` assumption baked into `client.create`). Prior pass (charge sourcing, 2026-06-02): adds the `sourcing.*` domain (5 tools) — prospect candidates sourced from the Companies House mortgage/charges register (Product 199) via the new `charges-service`. Flow: `sourcing.searchLenders` (disambiguate a known lender) → `sourcing.fromLender` (the companies that lender has charged, CH-enriched + deduped vs the book) → triage via `sourcing.list` / `sourcing.setState` → `sourcing.promote` into the prospect pipeline. Candidates live in `sourcedCompanies`, NOT clients. Prior pass (knowledge surface, 2026-05-31): adds `intelligence.getKnowledgeItemsByClient` (read structured facts back) + `intelligence.updateClientIntelligence` (bulk-patch the clientIntelligence doc — prospect-intel uses it for Output #2: identity + key people + summary), and `prospect.getDeepContext` now returns `knowledgeItems` (the AI/operator facts were previously write-only, invisible to the read path). Prior pass (client context capture, 2026-05-31): adds `intelligence.appendContext` (operator running-context md) + `note.create` / `note.update` / `note.listByClient` / `note.listByProject` (the notes lane, previously unexposed) — the agent surface for the `client-context-capture` skill (operator primary-knowledge input). Prior pass (outreach gating, 2026-05-30): adds `client.markOutreachReady` / `client.clearOutreachReady` / `client.listOutreachReady` — the operator "accept → ready for outreach" gate between intel-only `prospect-intel` and the new `outreach-draft` skill (see [`skills/prospect-pipeline-gates.md`](./skills/prospect-pipeline-gates.md)). Prior pass (corporate-structure): adds structure.renderChart (render a StructureGraph to a styled ownership-only SVG + data-URI + high/med/low verdict) + companies.mapGroup (one-call group map: CH numbers + directors + appointmentsLinks — the starting point for the corporate-structure walk). Prior pass (P4 docgen): adds document.generate. Prior pass (lender-tier-conflict): adds companies.getLenderTierConflict. Prior pass (prospect-schemes): adds companies.getProspectSchemes + upsertProspectScheme. Prior pass (corporate-group charges): adds companies.getGroupCharges. Prior pass (post-v1.4 Sprint K): contact.create/update, companies.searchCompaniesHouse, companies.getOfficerAppointments.
...
| "Who are the directors / people behind {synced company}?" | `companies.getOfficers` |
...
| `prospect.promoteStage({clientId, toStage})` | **Pipeline-stage promotion (2026-07-13).** Move a prospect between the operator's 5 MANUAL pipeline stages (cold_outreach / warm_pre_meeting / warm_post_meeting / pre_qualification / qualified) — the axis behind the stage-by-stage `/prospects` dashboards and the `prospecting/` stage workspaces. Any direction (force), exact UI parity; logs `prospectStageEvents` (kind `pipeline_stage`, reason `manual`) so entered-this-month KPIs stay exact. Separate axis from `prospectState`; does NOT change `clients.status` — graduating out of the pipeline is `client.activate`. |
| `prospect.setQualSubStage({clientId, subStage})` | **Sub-stage ladder step (2026-07-13).** Set the prospect's current ladder step: pre-qualification ladder (modelling_required → modelling_review_required → qualitative_feedback_required → feedback_given → feedback_discussed) or qualified ladder (terms_requested → terms_presented → progression_to_credit → formal_dd → credit_approved). Which ladder applies is gated by `pipelineStage` — promote the stage first; the tool doesn't validate the pairing (UI parity). Logs `prospectStageEvents` (kind `qual_substage`). |

### `client.*` — Client workflows (alias of prospect; for active clients) (11)
...
| `client.dismissNeedsActionFlag({clientId, kind, sourceReplyEventId?})` | **Backlog reset (2026-07-14).** Clear one needs-action flag (the `flaggedClients` triage section / "Waiting on you" chip) after the operator made the decision it was asking for. kind + sourceReplyEventId identify the flag. |
...
### `lender.*` — Lender intelligence + matching + submission requirements (11)
...
| `lender.create({name?, promoteFromCompanyId?, hubspotCompanyId?, aliases?, sourceDocumentIds?, ...})` | **3 modes** (Sprint K); mode 3 is now an **UPSERT** (2026-07 lender-DB hardening): (1) `promoteFromCompanyId` (Convex id) promotes an existing companies-table row into a lender — auto-inherits metadata + links synced contacts; (2) `hubspotCompanyId` (string) when you only have the HubSpot id from `contact.hubspotCompanyIds[0]` — resolves + promotes; (3) `name` alone for naked creation. In mode 3, before inserting it runs a **conservative normalized-name match** (lowercase, strip punctuation, drop trailing legal suffixes — equality only, never fuzzy) across every existing lender's `name`/`companyName`/`aliases`; on a match it **patches that row instead of duplicating** (unions `aliases` + `sourceDocumentIds`, appends a dated note, fills missing website/companyName/country) and returns `{status:"deduped", deduped:true, lenderClientId}` with the EXISTING id — so a document-ingestion wave re-encountering 'Funding 365' / 'Downing LLP' collapses onto the first row. Pass `aliases` (alternate names — also feed the knowledge-layer lender matcher) + `sourceDocumentIds` (evidence). Most lenders are already in HubSpot via contact sync — prefer modes 1/2 when possible. To collapse EXISTING duplicate rows, use `lender.merge`. |
| `lender.merge({fromClientId, toClientId, dryRun?})` | **2026-07 lender-DB hardening.** Collapse a DUPLICATE lender row (`fromClientId`) into the canonical survivor (`toClientId`) — the operator-hygiene fix for duplicates `lender.create` couldn't auto-catch (Funding 365 ×2, Downing vs Downing LLP, Paragon vs Paragon Bank). Both must be `type='lender'` and distinct. Repoints knowledge atoms + facilities + appetite signals + chunks (via `atoms.mergeEntities`), then flat CRM refs (contacts/documents/tasks/notes/meetings/…) + `projects.clientRoles`, then unions the lender row fields (aliases — incl. from-row's name/companyName — sourceDocumentIds, notes with a merge marker, missing scalars), and finally soft-deletes the from-row (`deletedReason=merged_into_<toId>`). `dryRun:true` (recommended first) returns validation + would-be repoint counts + field-merge preview WITHOUT writing. |
...
| `lender.getDocuments({lenderClientId, limit?})` | The lender's **document evidence trail**: every document evidencing the lender, federated from 4 lanes — the lender row's `sourceDocumentIds`, lender knowledge atoms (via observations), facility-book atoms (term sheets / facility agreements — the later-stage deal paper), appetite sources. Each doc: fileName, type, **summary**, client/project scope, `atomCount` (knowledge pulled from it for this lender), `via[]` lanes. Newest first, cap 100. Answers "which documents did these terms come from?"; powers the Lenders-tab Documents view. |
...
### `cadence.*` — Outreach cadence flow (19)
...
| `cadence.reactivate({cadenceId, newNextDueAt?})` | **Outreach triage (2026-07-14).** Recovery for `stalledCadences`: clears all three silent stall states (intel hold, 3-strike auto-deactivation, pause), sets `isActive` true, optional reschedule. Refuses deliberate stops (cancelled / denied / needs_contact) with the right redirect. For intel_hold, re-validate intel first. |
| `cadence.adoptManualSend({packageId, sentAt, preset?, gmailMessageId?})` | **Reconciliation autofit (2026-07-14).** Operator sent touch 1 THEMSELVES outside the system: marks touch 1 fired-externally at the real send date (never re-sends), REFITS unfired follow-ups onto the preset schedule anchored there (past dates pushed forward), logs the touchpoint (idempotent on gmailMessageId), stamps lastOutreachSendAt + prospect state. Approval status unchanged — pending packages still need the normal approve; already-approved ones auto-send on the new dates (warn the operator). |
| `cadence.denyPackageBatch({packageIds, reason?})` | **Backlog reset (2026-07-14).** Deny ≤25 packages in one call — every touch denied + inactive, none ever fire. For stale drafted outreach (e.g. prospect already emailed manually outside the system — sending now would double-email). Itemise + explicit yes first; per-item safe. Fresh outreach later via `outreach-draft`. |
| `cadence.listUpcoming({daysAhead?, limit?})` | **Outreach triage (2026-07-14).** The OUTBOX: every active touch due inside the horizon (default 7d, max 90) sorted by fire date, each with an honest `fireStatus` — `scheduled`/`due_now` (fires on the next 5-min tick) vs `paused`/`blocked_package_pending`/`blocked_no_contact_email` (will NOT send, and why). Overdue-but-blocked rows answer "why didn't this send?". |
| `cadence.approvePackageBatch({packageIds})` | **Outreach triage (2026-07-14).** Approve ≤25 packages in one call — full approvePackage semantics each (no-contact guard, forward-only stage write) with ONE dispatcher kick at the end. Per-item safe: guard failures land in `results[]` with `ok:false`. Itemise the packages to the operator before calling. |
...
### `outreach.*` — Email draft staging + triage + metrics + prospecting inbox (7)
...
| Tool | Purpose |
|---|---|
| `outreach.triageQueue({})` | **Outreach triage (2026-07-14).** THE cross-prospect triage read: every open outreach action in one call — `pendingPackages`, `needsContact`, `replyDrafts`, `otherApprovals`, `failedSends`, `unroutedReplies`, `deadEndReplies` (matched no contact/prospect — otherwise invisible), `stalledCadences` (intel_hold / auto_deactivated_failures / paused), `flaggedClients`, `staleIntel`. Trimmed rows (no bodies) — drill in with `approval.get` / `reply.get` / `cadence.get`. First call of every `/outreach` session. |
| `outreach.metrics({sinceDays?})` | **Phase 2 (2026-07-15).** Outcome metrics over a window (default 90d): sends (incl. backfilled manual), substantive replies (OOO excluded from rates), contact-level + send-level response rate, **touchesPerEarnedReply**, and a byTemplate table (replies attributed to the latest fired touch before the reply; legacy sends = 'untagged'). Capped/windowed — `capped` flags mean floors. Run the backlog reset first for honest baselines. |
| `outreach.prospectingInbox({stage?, direction?, includeNonProspects?, windowDays?, limit?})` | **THE prospecting inbox (2026-07-17).** Org-wide, client-linked mail in ONE feed, both directions, newest first: inbound = replyEvents with a linkedClientId; outbound = gmail email touchpoints — in-app sends AND manual Gmail sends (the SENT poller captures those every 5 min, deduped by Gmail message id). Trimmed rows (subject + 160-char snippet, client + stage + contact, OPERATOR — whose mailbox sent/received it, intent/resolved, hasAttachments) — drill in with `reply.get`. Filter by pipeline stage / direction; prospects-only by default. A recency surface (default 45d window), not an archive. |
| `outreach.prospectingKpis({sinceDays?, includeNonProspects?})` | **Prospecting funnel KPIs (2026-07-17).** Windowed counts (default 30d), totals + per pipeline stage: outboundSent / inboundReceived / meetingsHeld / meetingsUpcoming + uniqueProspectsContacted / uniqueProspectsReplied + a byOperator breakdown (who is doing the prospecting — mail attributes to the mailbox owner, meetings to the calendar owner). Meetings come from the CALENDAR ATTENDEE MATCHER: a Google Calendar event counts for a prospect when ≥1 attendee email resolves to one of its contacts (operators + cancelled + internal-only events excluded). Measures the funnel regardless of how mail was sent — `outreach.metrics` stays the CADENCE-pipeline lens (touches per earned reply, per-template). |
...
### `reply.*` — Inbound reply visibility + classification (6)
...
| `reply.get({replyEventId})` | Single reply with full body + classification + cancelledCadences. Gmail rows carry `attachments` metadata (filename/mimeType/size/partId/inline) — captured at ingest + backfilled onto historical rows 2026-07-16. |
| `reply.listAttachments({replyEventId?} or {gmailMessageId?})` | List an inbound email's attachments LIVE from Gmail (works for rows ingested before attachment capture, and for raw Gmail / RFC822 message ids never ingested). Returns `{gmailApiId, subject, fromEmail, attachments:[{filename, mimeType, sizeBytes, partId, inline}]}`; `inline:true` = embedded signature images, usually skip. Reads the reply's OWNING user's mailbox (raw ids read the caller's). File one into Drive with `drive.saveEmailAttachment`. |
| `reply.resolveBatch({replyEventIds, resolutionNote})` | **Backlog reset (2026-07-14).** Mark ≤100 replies HANDLED — they leave listUnrouted / triageQueue / the digest but keep history (resolvedAt/By/note; "handled by X" shows in the app). For replies already answered manually via Gmail or not actionable. Itemise + explicit yes first. |
...
| `document.extractText({documentId})` | **Harness classification step 1 (2026-07-07; vision fallback 2026-07-08; V1.2 parsedName signal 2026-07-08).** Returns `{text (≤120K, truncation noted), fileName, mimeType, contentChecksum, source, method ('parser'\|'vision'), parsedName, alreadyClassified, alreadyAtomized}` so YOU classify. `parsedName` is the V1.2 file-naming-standard parse of the fileName (`{scheme, docType, documentDate?, origin{role,party?}, status?, version?, reissue?, filingDate, confidence:'full'\|'partial'}`, null for non-standard names; schema of record `src/lib/naming/filename_schema.json`) — a `confidence:'full'` `docType` is a STRONG classification prior (the schema's docTypes map gives its `appFileType`); `'partial'` is a hint to verify against content. Text-layer docs are parsed server-side, ZERO LLM (`method:'parser'`). Image documents (PNG/JPG term sheets) and scanned image-only PDFs — which the parser returns EMPTY for — are transcribed faithfully via Claude vision (`method:'vision'`; treat as best-effort OCR, `[illegible]` marks unreadable regions), so they still contribute atoms instead of zero. For a pending Drive doc it fetches the bytes from Drive, caches them, and CLAIMS the mirror row (`processing`) so the automatic pipeline doesn't race you — finish step 2 within ~30 min or the claim reclaims and the API lane takes over. Keep `contentChecksum` for step 2. `alreadyClassified: true` → skip in bulk passes. This is the SUBSCRIPTION-COST lane for bulk/onboarding classification; changed-file re-processing stays on the API pipeline. |
...
### `approval.*` — Operator approval queue (10)

| Tool | Purpose |
|---|---|
| `approval.listPending({entityType?, limit?})` | **THE session-opening read for chat-first approvals (2026-07-17).** Everything pending, org-wide, any entity type, newest first — trimmed rows (summary, entityType, client, requestedAt; full draft via `approval.get`). The loop: list → present each item in chat → operator's explicit yes → `approval.approve` / `approvalBatch` (executor fires immediately). The app's /approvals page is an alternative surface, never a requirement. |
...
| `approval.retry({approvalId})` | **Outreach triage (2026-07-14).** Re-queue an `execution_failed` approval for a real retry (kill switch off / token reconnect / transient error) — the operator already approved the content, so the same executor re-runs without a re-draft. No-op-safe: `{ok:false, reason:'not_failed_*'}`. |
| `approval.rejectBatch({approvalIds, reason?})` | **Backlog reset (2026-07-14).** Reject ≤50 pending drafts in one call — nothing sends, drafts discarded. Counterpart of approveBatch for clearing stale staged sends. Itemise + explicit yes first; per-item no-op-safe. |
| `approval.approveBatch({approvalIds})` | **Outreach triage (2026-07-14).** Approve ≤50 pending approvals in one call and fire each executor (same path as `approval.approve`). Per-item no-op-safe: missing/non-pending rows land in `skipped[]`. RULE: itemise the batch to the operator (recipient, subject, what fires) and get an explicit yes first. |
...
### `intel.*` — Intel freshness (1)

| Tool | Purpose |
|---|---|
| `intel.revalidate({clientId, companyNumber?, reason?})` | Cheap diff-focused check that a prospect's prior full intel still holds (new CH charges, status change, new planning/scheme activity, news). Returns `still_valid` \| `materially_changed`; on the latter the prospect is flagged for an intel refresh (`intelAttentionAt`). Lightweight counterpart to a full prospect-intel re-run — the cadence dispatcher runs it automatically before a touch fires after a >30-day gap, and a booked meeting on >7-day-stale intel raises the same flag. |

### `intelligence.*` — Structured intelligence reads + single-fact writes (7)

| Tool | Purpose |
|---|---|
| `intelligence.getClientIntelligence({clientId})` | **DEPRECATED (knowledge cutover 2026-07-11)** — fallback-only read for not-yet-atomized clients; prefer the deep-context `graph` section + `atoms.search` / `graph.*`. The clientIntelligence row for a client. Subsumed by `client.getDeepContext.clientIntelligence`. |
| `intelligence.getProjectIntelligence({projectId})` | **DEPRECATED (knowledge cutover 2026-07-11)** — fallback-only read for not-yet-atomized clients; prefer the deep-context `graph` section + `atoms.search` / `graph.*`. The projectIntelligence row for a project. Subsumed by `project.getDeepContext.projectIntelligence`. |
...
### `touchpoint.*` — Outreach activity log + manual backfill (4)

| Tool | Purpose |
|---|---|
| `touchpoint.logManualSend({entries[]})` | **Reconciliation backfill (2026-07-14).** Log ≤50 outside-the-system sends (generic Gmail tool) as real outbound touchpoints: history row + lastOutreachSendAt stamp + prospect-state advance, per entry. Idempotent on gmailMessageId. For sends matching a drafted package's touch 1, use `cadence.adoptManualSend` instead. |
...
### `companies.*` — External company sync (10)
...
| `companies.getOfficers({companyNumber})` | Read back the **officers + PSCs** of an already-synced company (error `company_not_synced` otherwise — sync first). Returns `{companyName, activeOfficerCount, officers[{name, officerRole, appointedOn, resignedOn, isActive, occupation, appointmentsLink}], psc[]}`, active officers first. The **name-seed for contact discovery**: `apollo.findEmail` needs a named person (no company-wide people search), so the lender-intel enrich gauntlet and prospect-intel's director step read names here, enrich each, and persist keepers via `contact.create`. |
...
### `deal.*` — Mirrored HubSpot deals (selection reads) (1)

| Tool | Purpose |
|---|---|
| `deal.listByStage({pipelineId, stageId, limit?})` | **Cold-reachout selection (2026-07-15).** Mirrored HubSpot deals in one pipeline stage, each joined to its app prospect with dedupe/readiness flags: `appClient` (stage/state/lastOutreachSendAt), `alreadyWorked` (send evidence — skip in cold selection), `contactWithEmail`. Both ids required (dealstage ids are per-pipeline). Freshness = the mirror sync, not live HubSpot. |
...
| `drive.saveEmailAttachment({replyEventId?/gmailMessageId?, filename, partId?, targetFolderId, newName?, importToLibrary?})` | **Write — approval-gated.** Stage copying an attachment from an inbound Gmail email into a mirrored Drive folder. Same double gate. At execute time the bytes are fetched from the MAILBOX OWNER's Gmail (never stored in the app; attachment handles are re-resolved live — nothing stale is staged), uploaded into the target folder, and echoed into the mirror. Check the email first with `reply.listAttachments`; pass `partId` to disambiguate duplicate filenames. `importToLibrary:true` ALSO imports the uploaded file as a metadata-first document (extraction cost applies; needs an effective client mapping — skipped with a reason otherwise). The upload does NOT trigger folder auto-import even when armed (the echo-guard means the poller never sees it as new) — `importToLibrary` is the only import lane. |

### `atoms.*` / `graph.*` / `facilities.*` — Knowledge Layer (Spec 2) (20)
...
| `atoms.search({query, clientId?, subjectType?, status?, limit?, includeProspectScoped?, includeChunks?, chunkLimit?})` | TWO RESULT LANES in one call. `results` = ATOMS: HYBRID search over atom statements — full-text (Convex search index) + semantic vector similarity (Voyage `voyage-finance-2` embeddings), fused with reciprocal-rank fusion (RRF) — matches on MEANING as well as exact terms (e.g. "how leveraged is the scheme" surfaces LTGDV/loan atoms with zero shared words), and each hit carries a `lane` marker (text / vector / both) with `counts.{textLane, vectorLane, merged}`. `chunks` = PROSE PASSAGES: the same hybrid+RRF over the narrative dual index (`documentChunks`, the read side of `atoms.upsertChunks`) — each hit carries documentId + parent doc `{displayName, fileName, fileTypeDetected}` + `locator` (page/sheet/section) + chunkIndex + rrfScore; text trimmed to ~700 chars (`truncated:true` when cut). Atoms answer WHAT (discrete facts with provenance); chunks answer the nuance around it (caveats, conditions, reasoning in legal opinions/reports). `includeChunks` defaults true (`false` = atoms only); `chunkLimit` default 8, max 20; `chunkCounts` mirrors the lane counts. Default returns LIVE atoms only (active + contested). Atom hits carry resolved subject/object entity names, objectLiteral, confidence, primarySourceType + observation count; the atomId is the drill-down handle. START a graph walk here: resolve the entity off the top hit, then `graph.expandEntity`. Contested hits: present BOTH values, never pick silently. `includeProspectScoped` defaults true (the agent lane sees everything — spec §14b.6a); `false` hides atom hits whose owning clientId is a prospect-status row, `counts.prospectScopedHidden` reports how many. Both lanes degrade to full-text alone (`vectorLaneDisabled:true`) if embeddings are unavailable. |
| `atoms.createCandidate({mentionText, guessedType, contextSnippet?, sourceDocumentId?, clientId?})` | Mint (or reuse) a PROVISIONAL entity for a mention that resolves to no roster id — the repair path for `unresolved_subject`/`unresolved_object` rejects. Anchor the atom to the returned candidateId (`subjectType`/`objectEntityType: "candidate"`) and resubmit: **facts are never dropped**. Dedup built in — the same normalized mention (lowercase, punctuation-stripped) + guessedType reuses ONE row (`reused: true`); a candidate that ALREADY RESOLVED returns `{status: "resolved", resolvedType, resolvedId}` — anchor to that real entity directly. ALWAYS pass `clientId` when known (scope hint, stored inside contextSnippet as `client:<id>\|<snippet>`): it is what lets the worker scan the right contact roster and give Apollo a company context. The 2-hourly enrichment worker resolves candidates (companies → CH exact-name search + full sync; people → client contacts, then Apollo) and re-points all referencing atoms through the identity machinery (duplicates merge, value clashes go contested). |
| `atoms.listCandidates({status?})` | List provisional entities with referencing-atom counts (`referencingAtoms.{asSubject, asObject, total}`), `enrichmentAttempts`, parsed `scopeClientId`. Filter: pending (triage view — attempts=3 means the worker gave up and it is waiting on the operator; NEVER auto-dismissed) / resolved (tombstones with resolvedToType/resolvedToId) / dismissed. |
| `atoms.dismissCandidate({candidateId})` | Operator hygiene: mark a candidate dismissed — not a real resolvable entity (typo, generic phrase); the worker never chases it again. Anchored atoms survive (flagged unconfirmed via the candidate reference); re-encounters reuse the dismissed row rather than resurrecting it. Errors on resolved candidates (the tombstone must survive). The ONLY dismissal path — the worker just stops retrying after 3 failed attempts. |
| `atoms.mergeEntities({entityType, fromId, toId, reason, toType?})` | Operator hygiene: collapse a DUPLICATE existing entity into its canonical twin on the KNOWLEDGE GRAPH (a client created twice, a `(175)`-suffixed contact, a company under two CH rows). Re-points every knowledge-side ref `fromId`→`toId` (both the same `entityType`) and routes each atom through the SAME identity machinery Phase-2b candidate resolution uses: a re-pointed atom that now duplicates a live atom on the target MERGES (observations move to the survivor, corroboration bumps, dupe superseded); a value clash goes CONTESTED (never a silent double). Also re-scopes the denormalized refs the subject/object re-point misses — atoms.clientId/projectId, the facilities mirror columns (lender/borrower/company/project), documentChunks scope tags, appetiteSignals.lenderClientId. KNOWLEDGE-GRAPH ONLY — **CRM-BLIND**: does NOT reassign CRM tables (contacts/documents/tasks/notes/…) and does NOT delete/soft-delete the source row. For client duplicates run `migrations/mergeDuplicateClients` (its atom-blind mirror) for the CRM side, then remove the source. `entityType`: client / project / contact / company / facility / candidate — for `candidate` this RESOLVES it to a real entity (pass `toType`; `toId` is the real entity's id). Writes an `auditLog` row; returns `{repointed, merged, contested, scope:{atomsRescoped, chunksRescoped, facilitiesRescoped, appetiteRescoped}}`. |
...
| `graph.sharedNeighbors({entities, via?})` | The "what connects these?" primitive: expand each of the 2–5 inputs into a bounded structural GROUP (client → role projects + facilities + CH group companies; project → facilities + their SPV client; company → its group client(s) + mapped sibling companies; contact/facility/candidate → self only; ≤25 members/input, `capped` per input in `groups`), then intersect the UNION of each group's members' one-hop federated neighborhoods → nodes reached by ALL inputs (a node counts for an input if ANY group member has a direct edge). Per-input connections `{fromInput, groupMember?, predicate, direction, provenance}` — `groupMember` set ⇒ the link is input→member→node (the group hop). Closes the lender→facility→project→developer gap. Only the inputs themselves are excluded; a group member the other side reaches is a valid shared node. `via`: people / companies / lenders / any. Run as the prospect-connection check ("shares a director with client X?") after prospect-intel creates entities (spec §14b.4). |
...
| `facilities.create({lenderClientId, projectId, borrowerClientId?, tranche?, amountGBP?, interestRate?, maturityDate?, status?})` | **Manually add a facility** to a lender's book — operator lane for deals the documents haven't evidenced. Flagged `createdFrom:'operator'` (orange provenance dot in the UI); minter-compatible dedupeKey, so a later document-evidenced mint lands on this row (returns `facility_exists` + the id if the row already exists). NOT for document-evidenced facilities — atomize the document instead so provenance is real. |
| `facilities.updateTerms({facilityId, amountGBP?, interestRate?, maturityDate?})` | Update a facility's **terms** — pass only the fields to change. On pipeline-minted rows these columns are atom mirrors: the operator value holds until NEWER document evidence rematerializes the row (operator number = current truth, later executed doc = newer truth); operator-created rows are never rebuilt. For document-evidenced changes prefer atomizing the document. |
| `facilities.setStatus({facilityId, status})` | **Operator override** for facility lifecycle status (indicative/live/repaid/defaulted) — permits ANY transition incl. downgrades ("I know this repaid"). Status is not an atom mirror; rematerialisation never clobbers it, later pipeline stamps still only upgrade. |
| `facilities.audit({projectId?, execute?})` | **2026-07 lender-DB hardening.** Find (and optionally fix) FRAGMENTED facilities — the multiple rows free-text tranche descriptors used to mint for one negotiation (Allica Bank: 8 rows on one project from successive quote revisions). Groups facilities by project + lender + normalized tranche (`senior`/`mezzanine`/`bridge`/`equity`/`single`); any group with >1 row is a fragment cluster of what should be ONE facility. Dry-run (default) returns each cluster's rows (id/tranche/amount/status/atomCount) + the suggested canonical row (most attached atoms, then richest mirrors). `execute:true` collapses fragments into the canonical via the completed facility merge path (fills mirrors, recomputes dedupeKey under the enum scheme, deletes the fragment, rematerializes) and returns the merge count. `projectId` scopes to one project; omit for the whole corpus. External/unrostered facilities (no lender id) are excluded. Pairs with the tranche-enum + status-stamping (indicative/live from source-doc type) now applied at mint time. |
| `graph.overview({maxNodes?, maxEdges?})` | The ORG-WIDE snapshot (the atlas board): `{nodes, edges, counts}` for EVERY entity and edge in one call — all clients (`clientType` lender/borrower/developer, `clientStatus` active/prospect) and projects, plus every contact/company/facility/candidate with ≥1 atom or structural edge. Node keys and edge endpoints share `<type>:<id>`; nodes carry `atomCount`/`contestedCount` (live atoms as subject) + `degree`; edges carry `kind` (atom/native), `status`, `atomId?` (the drill-down handle), `confidence?`/`salience?` (atom lane), `corroborated?` (native mirror agreed). Dedupe per (from, to, predicate): atom wins over native; a contested duplicate is never hidden. Bounded: `maxNodes` ≤2500 / `maxEdges` ≤6000 (lowest-degree contact/company/candidate leaves drop first; `counts.truncated` flags it); `counts` also carries org-wide `atoms`/`contested` totals + node counts `byType`. Use for whole-book questions — lenders fanning across clients, cross-client clusters, contested hotspots — then drill with `graph.expandEntity`. No prospect-scope filter (the everything view). Served from a CACHED snapshot (the org-wide walk outgrew a single query execution): rebuilds first only when the cache is older than ~5 min, and `builtAt` (epoch ms) rides along so the snapshot's age is visible. |
...
### Pattern: Email attachment → Drive folder

```
1. reply.listByClient / reply.listUnrouted — find the inbound email (rows since
   2026-07-16 show attachments inline; reply.get carries the same)
2. reply.listAttachments({replyEventId}) — confirm filename/partId LIVE from Gmail
   (required for older rows; skip inline:true signature images)
3. drive.listFolders — navigate to the destination folder (client-mapped subtree
   if the file should later import under that client)
4. drive.saveEmailAttachment({replyEventId, filename, partId?, targetFolderId,
   importToLibrary: true?}) — stages a PENDING approval; needs the /settings/drive
   write-back switch ON
5. Operator approves at /approvals → bytes copy Gmail → Drive, mirror updated;
   with importToLibrary the file also lands in the library (metadata-first,
   extraction follows). Without it, import later via drive.importFiles — the
   upload never auto-imports, even in an auto-import-armed folder.
```
