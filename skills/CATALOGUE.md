...
### `caseStudy.*` — Deal Book case-study index (5) — ⚠️ NOT LIVE
> **⚠️ Not on the live server** (verified via `meta.listTools`, 2026-07-16). These tools exist only on the unmerged `feat/deal-book-case-study-index` branch of rockcap-v2 — Convex deploys are whole-tree snapshots, so deploying any other branch removes them. Do not reference from skills until that branch merges + deploys.
...
### `dealBook.*` — Deal Book portfolio aggregates (1) — ⚠️ NOT LIVE
> **⚠️ Not on the live server** (verified via `meta.listTools`, 2026-07-16). These tools exist only on the unmerged `feat/deal-book-case-study-index` branch of rockcap-v2 — Convex deploys are whole-tree snapshots, so deploying any other branch removes them. Do not reference from skills until that branch merges + deploys.
