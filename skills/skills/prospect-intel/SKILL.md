...
9. **Cross-reference checks**. Follow `references/web-research-playbook.md` Phase C — internal knowledge lookups for prior connections, address cross-check, sister entity check. Read graph-first: `atoms.search` (hybrid fact + chunk retrieval) and `graph.sharedNeighbors` for connection candidates; fall back to `intelligence.getClientIntelligence` only where the relevant entity's graph is empty (not yet atomized). Output enriches sections 3 and 4.
...
10. **Persist intelligence**. Persist across the structured stores, then build the report:
    ...
    - **Discrete facts (`knowledgeItems`)** — write specific data points via `intelligence.addKnowledgeItem` (the lender-DNA summary, the classification, related entities, and any extracted figures). Downstream reads go graph-first (the `prospect.getDeepContext` `graph` section + `atoms.search`); `intelligence.getKnowledgeItemsByClient` / `prospect.getDeepContext.knowledgeItems` remain the fallback read surface for not-yet-atomized prospects.
...
- `intelligence.updateClientIntelligence`, `intelligence.addKnowledgeItem`, `intelligence.searchLenders` — writes + lender search for steps 9, 10 (people are enriched per-person via `apollo.findEmail` + `contact.create` in step 8, not a people-search tool). Reads for steps 3 and 9 go graph-first (`prospect.getDeepContext` graph section, `atoms.search`, `graph.sharedNeighbors`); `intelligence.getClientIntelligence` is fallback only, when the graph section is empty (prospect not yet atomized)
