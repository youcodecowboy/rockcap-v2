...
6. **Compose the four touches.** Load `references/template-mapped-reachout.md`, select the canonical template by lender DNA, write the touch-1 hook via the `compose-outreach-hook` sub-skill (`../../sub-skills/compose-outreach-hook.md`) and the hook ladder (`../../shared-references/hook-ladder.md`), in Alex's voice (`../../shared-references/rockcap-outreach-voice.md`). Compose all four touches per the `## Cadence package` section above. Geographic hooks draw on `../../shared-references/rockcap-regional-activity.md` + `sender-geography.md`.
   ...
   **Mutual connections (Spec 2 §14b.6b, 2026-07-07).** The intel's `## Graph connections` section (when present) lists what links this prospect to the RockCap graph, each hit marked public record or internal. You MAY reference a connection hit in an intro/hook line **only when its provenance is public record** — per the operator decision: *"outreach-draft may use connection hits in intro lines ONLY when the cited atoms'/edges' provenance is public record (`sourceType: companies_house` or native CH edges) — client-document-derived knowledge informs strategy but is never cited to a prospect. Machine-checkable at draft time via sourceType."* Internal hits (anything derived from another client's documents, briefs, or emails) shape the angle, the template choice, and the timing — silently. They never appear in the text.

   - Good: "I saw you and James Carter were on the Bayfield Homes board together — we arranged their last two facilities." *(CH officer register — public record; verifiable by the recipient.)*
   - Bad: "I understand your Wellington Road scheme is targeting a £4.2M GDV." *(That figure came from another client's appraisal — client-document-derived; citing it leaks a client's confidential knowledge to a prospect. Use it only to pick the deal-size band and the template.)*
