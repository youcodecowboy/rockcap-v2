...
| `onboard/` | **v1** | **NEW 2026-07-17** — the `/onboard` ACTION command: end-to-end client onboarding (client + contact-per-key-person → compact CH intel → `project.create` per scheme → Drive folders with in-chat approval gates → `drive.saveEmailAttachment` doc filing per the naming standard with `importToLibrary` → gate 2 `approval.approveBatch` → harness classify + atomize → offer `setAutoImport`). Canonicalises the Edgefold flow; the /approvals page is never required. |
| `cold-reachout/` | **v1** | **NEW 2026-07-15** — the `/cold-reachout N` ACTION command (Phase 1 of `docs/05`): select N from mirrored Weekly Targets (`deal.listByStage`, dedupe via alreadyWorked) → gate 1 (approve list) → orchestrates `prospect-intel` + `outreach-draft` per prospect (templateKey stamped) → jittered Friday/Saturday send-window scheduling → gate 2 (`cadence.approvePackageBatch`) → proposes HubSpot stage moves (write-back is Phase 3). |
| `outreach-triage/` | **v1** | **NEW 2026-07-14** — the `/outreach` control loop: `outreach.triageQueue` + `cadence.listUpcoming` → ranked cross-prospect checklist (replies, drafts, failed sends, pending packages, silent stalls, data fixes) → operator-confirmed batches (`approval.approveBatch` / `cadence.approvePackageBatch` / `approval.retry` / `cadence.reactivate`) → re-read + delta. Spans lifecycle steps 1.5–4 horizontally; pairs with the stage-workspace session digest (see `prospecting/CLAUDE.md`). |
