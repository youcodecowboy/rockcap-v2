# Source material: the "Metaprompt" (captured 19/07/2026)

External prompt-generator prompt Rayn rated highly (sourced from an Instagram post; full text
verified against deepwriting.ai's copy — matches the on-screen version word for word). NOT a
proven exemplar — it has not run in the field for us. It lives here as source material: its
principles are distilled into the SKILL.md self-check; consult the full text below when
building a prompt for a verification-heavy or error-prone task.

## Distilled principles (already in SKILL.md self-check)

1. Decompose complex tasks into smaller subtasks.
2. "Fresh eyes": never let the same expert/pass both create and validate a solution.
3. Iterative verification for tasks likely to produce errors or hallucinations.
4. No guessing — the prompt must instruct the target to disclaim uncertainty rather than fill gaps.
5. Spawn a specialist persona (e.g. "Expert Python") for computation, run code rather than estimate.
6. Succinct format; ask the user only when an ambiguity would cause errors.
