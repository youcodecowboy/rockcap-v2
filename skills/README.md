...
1. **`CATALOGUE.md`** — every MCP tool (135 across 27 domains), grouped by domain, with "when to use" guidance and common-pattern cookbooks. **Start here for tool discovery.**
...
The cost of staying disciplined is small (a one-line edit per change). The cost of letting it drift compounds — at 135 tools and 22 skills, Claude Code can already only pick well if the documentation is accurate. (This is now also enforced mechanically: `tools/validate-skills.mjs` blocks any commit that references a non-existent tool — see `CLAUDE.md` + `skill-forge`.)
...
- **cadence management via MCP (2026-06-01)**: cadences were createable/pausable but not *configurable* from Claude Code. Added `cadence.applyPresetSchedule` (reconfigure a package's timing by intensity — light/moderate/aggressive), `cadence.listByClient` + `cadence.listByContact` (discover cadences to manage), and extended `cadence.update` to edit `cadenceType` + `scheduleConfig` (recurrence), not just touch content/date. Backend: extended `cadences.updateInternal` + new `cadences.applyPresetScheduleInternal` (userId-based for MCP auth). **120 MCP tools / 23 domains.**
- **spreadsheet extraction, Claude-side (2026-06-01)**: the deferred "deep spreadsheet extraction" reframed — instead of refactoring the batch-coupled server pipeline, the server just hands over the cells and the model extracts. New `/api/sheet-data` route + `document.getSheetData` (stored xlsx/csv → structured cells) + `document.saveIntelligence` (write structured fields with `templateTags` + sheet!cell provenance) + new **`deal-appraisal-extraction`** skill (read cells → reason out GDV/TDC/units/peak debt/LTGDV → persist). Sets up appraisal-template *re-population* (the reverse) as phase 2. **122 MCP tools / 23 domains. 22 skills.**
- **v1.4+ (pending)**: terms-package-build + ic-paper-drafter hardening; behavioural lender signal cron; **appraisal-template re-population** (figures → populated xlsx via exceljs — the reverse of deal-appraisal-extraction); substrate cleanups jotted in `.logbook/inbox.md` (frontend-vs-MCP divergence on `client.list`, empty `projectIntelligence` rows from legacy table, vocabulary drift, userId-optional internal mutations for skill contexts).
